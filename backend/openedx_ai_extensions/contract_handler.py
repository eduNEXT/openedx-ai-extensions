"""
Centralized communication contract for OpenEdX AI Extensions.
Provides mapping for both Errors and Successes to ensure a consistent API contract with the frontend.
"""
import logging
import litellm
...
# Maps technical identifiers to safe user-friendly messages and HTTP status codes.
ERROR_MAP = {
...
# --- Success Contract Mapping ---
# Maps success scenarios to standard codes. This allows the frontend to be "smarter"
# about how it handles different successful completions.
SUCCESS_MAP = {
    "SUCCESS": {
        "message": "Success", # Generic success
    },
    "SESSION_CLEARED": {
        "message": "Conversation history has been reset.",
    },
    "QUIZ_GENERATED": {
        "message": "Quiz questions have been generated successfully.",
    },
    "SUMMARY_GENERATED": {
        "message": "Content summary generated.",
    },
    "PROCESSING": {
        "message": "AI workflow has started processing.",
    }
}

def get_error_info(exception):
    """
    Map technical exceptions to ERROR_MAP entries.
...
def get_success_response(code, response=None, metadata=None):
    """
    Generate a standardized success response dictionary.
    """
    success_details = SUCCESS_MAP.get(code, SUCCESS_MAP["SUCCESS"])
    return {
        "status": "success",
        "data": {
            "code": code,
